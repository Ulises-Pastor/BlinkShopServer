...
class ExistenciasController
{
    ...
    {
        const respuesta = await pool.query('SELECT prendas.clave AS Clave, prendas.descripcion AS Prenda, tallas.id AS Talla, unidades AS Unidades FROM existencias LEFT JOIN prendas ON prendas.clave = existencias.clave_prenda LEFT JOIN tallas ON tallas.id = existencias.id_talla');
        res.json( respuesta );
    }
    ...
    public async listarPorPrenda(req: Request, res: Response ): Promise<void>
    {
        ...
        const respuesta = await pool.query('SELECT prendas.clave AS Clave, prendas.descripcion AS Prenda, tallas.id AS Talla, unidades AS Unidades FROM existencias LEFT JOIN prendas ON prendas.clave = existencias.clave_prenda LEFT JOIN tallas ON tallas.id = existencias.id_talla WHERE clave_prenda = ?', [clave_prenda]);
